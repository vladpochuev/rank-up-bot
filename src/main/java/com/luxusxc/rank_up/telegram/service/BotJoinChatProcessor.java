    ...
    public BotAction updateChatInfo(ChatMemberUpdated myChatMember) {
        ...
        return bot -> {
            ...
            if (commonMember && promotedToAdmin) {
                processJoinChat(chat, myChatMember);
            }
        };
    }

    private void processJoinChat(Chat chat, ChatMemberUpdated chatMember) {
        ChatEntity chatEntity = new ChatEntity(chat.getId(), chat.getTitle(), chatMember.getDate());
